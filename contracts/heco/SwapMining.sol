...
contract SwapMining is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocJoinAmount; // How many JOINs
    ...
    // Only tokens in the whitelist can be mined JOIN
    ...
    function getJoinReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getJoinReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        ...
        join.mint(address(this), joinReward);
        // Increase the number of tokens in the current pool
        pool.allocJoinAmount = pool.allocJoinAmount.add(joinReward);
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        address pair = IJoinFactory(factory).pairFor(input, output);
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocJoinAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocJoinAmount = pool.allocJoinAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
        ...
        join.transfer(msg.sender, userSub);
    }
    ...
    function getUserReward(uint256 _pid) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getJoinReward(pool.lastRewardBlock);
            uint256 joinReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocJoinAmount.add(joinReward)).mul(user.quantity).div(pool.quantity));
        }
        //Join available to users, User transaction amount
    ...
    function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256){
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IJoinPair(pool.pair).token0();
        address token1 = IJoinPair(pool.pair).token1();
        uint256 joinAmount = pool.allocJoinAmount;
        uint256 blockReward = getJoinReward(pool.lastRewardBlock);
        uint256 joinReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        joinAmount = joinAmount.add(joinReward);
        //token0,token1,Pool remaining reward,Total /Current transaction volume of the pool
        return (token0, token1, joinAmount, pool.totalQuantity, pool.quantity, pool.allocPoint);
    }
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IJoinFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IJoinFactory(factory).getPair(outputToken, intermediate) != address(0) && IJoinFactory(factory).getPair(intermediate, anchorToken) != address(0)) {
