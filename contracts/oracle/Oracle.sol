...
library JoinOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IJoinPair(pair).price0CumulativeLast();
        price1Cumulative = IJoinPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IJoinPair(pair).getReserves();
...
contract Oracle {
    ...
    function update(address tokenA, address tokenB) external {
        address pair = IJoinFactory(factory).pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'JOINOracle: PERIOD_NOT_ELAPSED');
